...
public class ShoutCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (cost > 0.0 && !bypassCost) {
            if (!VaultHook.hasEconomy()) { p.sendMessage("§c경제 플러그인이 필요합니다."); return true; }
            if (!VaultHook.economy().has(p, cost)) { sendInsufficient(p, cost); return true; }
            if (!VaultHook.economy().withdrawPlayer(p, cost).transactionSuccess()) { p.sendMessage("§c비용 차감 실패"); return true; }
            charged = true;
        }
        ...
        String built = TextUtil.colorize(prefix, true, true).replace("{player}", p.getName()) + msgBody;

        TextComponent comp = new TextComponent(TextComponent.fromLegacyText(built));
        TextComponent hoverComp = new TextComponent(
                TextUtil.colorize(hover, true, true)
        ...
        for (Player t : Bukkit.getOnlinePlayers()) {
            t.spigot().sendMessage(net.md_5.bungee.api.ChatMessageType.CHAT, comp);
        }
